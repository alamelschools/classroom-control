...
def send_sound_level(client_socket):
    ...
    p = pyaudio.PyAudio()
    stream = p.open(format=pyaudio.paInt16, channels=1, rate=44100, input=True, frames_per_buffer=512)  # Reduced buffer size

    while listening:
        data = np.frombuffer(stream.read(512), dtype=np.int16)  # Reduced buffer size
        volume_norm = np.linalg.norm(data) / 512
        decibel = 20 * np.log10(volume_norm + 1e-10)  # Calculate decibel level
        
        # Compress data before sending
        compressed_data = zlib.compress(str(decibel).encode())
        try:
            client_socket.send(compressed_data)
        except BrokenPipeError:
            break
        time.sleep(2)  # Send data less frequently
...
def student_mode():
    ...
    client_socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)  # Using UDP for lower latency
